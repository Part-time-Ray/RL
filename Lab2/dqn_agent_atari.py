...
class AtariDQNAgent(DQNBaseAgent):
	def __init__(self, config):
		...
		if self.network == "Dueling":
			self.behavior_net = DuelingAtariNetDQN(num_classes=self.env.action_space.n, channel=self.frame_stack)
			self.target_net = DuelingAtariNetDQN(num_classes=self.env.action_space.n, channel=self.frame_stack)
		else:
			self.behavior_net = AtariNetDQN(num_classes=self.env.action_space.n, channel=self.frame_stack)
			self.target_net = AtariNetDQN(num_classes=self.env.action_space.n, channel=self.frame_stack)
	...
	def decide_agent_actions(self, observation, epsilon=0.0, action_space=None):
		...
		if random.random() < epsilon:
			...
			with torch.no_grad():
				action = self.behavior_net(observation).argmax(1).long()[0].cpu()
				action = int(action)
