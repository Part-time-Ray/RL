...
if __name__ == '__main__':
	...
	for i in range(100):
		...
		state = preprocessor.step(next_observation)
	for i, s in enumerate(state):
		image = np.uint8(s * 255)
		cv2.imwrite(f"test_{i+1}.png", image)
...
class DQNBaseAgent(ABC):
	def __init__(self, config):
		...
		self.preprocessor = AtariPreprocessor(frame_stack=self.frame_stack)
		self.logdir = os.path.join(config["logdir"], self.game, self.network)
		...
		while self.total_time_step <= self.training_steps:
			...
			while True:
				...
				if terminate or truncate:
					...
					print(f"[{self.total_time_step}/{self.training_steps} ({self.total_time_step/self.training_steps*100:.1f}%)]  episode: {episode_idx}  episode reward: {episode_reward}  episode len: {episode_len}  epsilon: {self.epsilon}  learning_rate: {self.lr}")
					break
	...
	def evaluate(self):
		...
		for i in range(self.eval_episode):
			...
			while True:
				...
				frames.append(frame)
				action = self.decide_agent_actions(state, self.eval_epsilon, self.test_env.action_space)
				...
				# observation = next_observation
			if self.video:
				video_path = os.path.join("video", f"{self.game}", f"{self.network}")
		...
		if avg > self.best_eval_reward:
			...
			self.save(os.path.join(save_path, f"{self.network}_model.pth"))
		return avg
	
	def evaluate_one_episode(self, seed=42):
		frames = []
		...
		while True:
			frame = self.test_env.render()
			frames.append(frame)
			action = self.decide_agent_actions(state, 0, self.test_env.action_space)
			next_observation, reward, terminate, truncate, info = self.test_env.step(action)
			next_state = self.preprocessor.step(next_observation)
			total_reward += reward
			if terminate or truncate:
			...
			# observation = next_observation
		if self.video:
			video_path = os.path.join("video", f"{self.game}", "inference")
			os.makedirs(video_path, exist_ok=True)
			with imageio.get_writer(os.path.join(video_path, f"{self.network}_eval.mp4"), fps=30) as video:
				for f in frames:
					height, width = f.shape[:2]
					new_width = (width // 16) * 16
					new_height = (height // 16) * 16
