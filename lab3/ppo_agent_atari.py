...
class AtariPPOAgent(PPOBaseAgent):
	def __init__(self, config):
		...
		if len(config.get("load_model", "")) > 0:
			self.load(config["load_model"])
			print(f"Load model from {config['load_model']}")
	...
	def decide_agent_actions(self, observation, eval=False):
		...
		if eval:
			with torch.no_grad():
				action, logp, value, _ = self.net(torch.from_numpy(observation).to(self.device), eval=True)
		else:
			action, logp, value, _ = self.net(torch.from_numpy(observation).to(self.device))
		
		return action.cpu().numpy()[0], logp.cpu().detach().numpy()[0], value.cpu().detach().numpy()
	...
	def update(self):
		...
		for _ in range(self.update_count):
			for start in range(0, sample_count, self.batch_size):
				...
				# ???, ???, ???, ??? = self.net(...)
				_, logp, value, entropy = self.net(ob_train_batch, a=ac_train_batch)

				# calculate policy loss
				# ratio = ???
				# surrogate_loss = ???
				ratio = torch.exp(logp - logp_pi_train_batch)
				surr1 = adv_train_batch * ratio
				surr2 = adv_train_batch * torch.clamp(ratio, 1.0 - self.clip_epsilon, 1.0 + self.clip_epsilon)
